...
| 2 | Tool loop policy | Tool stalls, malformed calls, retries, and approval dead ends directly block task completion. | tool selection rules, retry policy, recovery prompts, approval escalation thresholds |
| 3 | Budget allocation | The harness needs to know when to spend on model calls, verifiers, VLM, retrieval, or subagents. | budget profiles, downshift rules, confidence thresholds, verifier/VLM spend policy |
| 4 | Verifier selection and routing | Verifier choice is the safety backbone, especially for visual/UI and risky code changes. | selector rules, verifier bundles, rerun policy, flake handling |
| 5 | Swarm scheduling | Swarm currently runs near evolution but is not yet driven by it. | subagent strategies, island assignments, budget weights, specialist profiles |
| 6 | Visual/VLM pipeline | Visual evidence is high value but easy to overtrust or underspecify. | capture strategy, VLM rubric, diff thresholds, OCR/PDF/image routing |
| 7 | Memory promotion and decay | Bad durable memory can poison future runs; good memory compounds. | promotion policy, decay rules, contradiction penalties, retrieval priority |
| 8 | MCP and capability trust | External tools and capability packages are trust boundaries. | trust tiers, quarantine rules, allow/deny policy, poisoning checks |
| 9 | Deep Research quality loop | Research tasks need citation, contradiction, novelty, and figure-evidence discipline. | source ranking, claim extraction prompts, contradiction checks, report templates |
| 10 | Evidence-gated approvals | Low-risk unattended evolution is useful, but unsafe self-approval would be dangerous. | auto-approval tiers, rollback requirements, human-required boundaries |
...
### 4. Verifier Selection And Routing
...
3. Add tool-loop policy evolution because tool stalls are direct task blockers.
4. Add budget allocation evolution because it controls spend across BES, VLM, verifiers, and swarm.
5. Expand verifier selector and visual/VLM evolution together because they share evidence and safety thresholds.
