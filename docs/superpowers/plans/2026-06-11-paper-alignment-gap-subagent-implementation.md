...
- [ ] Write tests proving every new feature defaults to disabled/offline/advisory.
- [ ] Implement shared quarantine/redaction helper for dashboard/report/A2A/model-assisted fields.
- [ ] Include negative tests for secret-shaped strings, absolute paths, traversal paths, oversize payload summaries, and external verification escalation attempts.
- [ ] Run:
...
- [ ] Commit with `feat: gate production organism capabilities`.
